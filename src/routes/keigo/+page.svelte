<script lang="ts">
	...
	function next(): void {
		...
		if (idx < rounds.length - 1) idx += 1;
			...
			scene = 'done';
			recordGameResult('keigo', score >= rounds.length - 1, score === rounds.length);
		}
	}
		...
		<article class="scene">
			...
			<div class="choices">
				{#each r.opzioni as c, i (i)}
					<!-- niente `disabled` dopo la risposta: bloccava i tocchi sulle
					     parole di InteractiveSentence (popup morto); pick() ha la guardia -->
					<button
						class="choice"
