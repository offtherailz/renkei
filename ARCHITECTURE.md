...
4. **Cinture giochi** (`gameBelts.ts` → localStorage `renkei_game_belts`): contatori per gioco `{played, clean}` aggiornati da `recordGameResult(gameId, clean)` alla fine di ogni partita. La cintura deriva dai contatori (bianca 1 partita, gialla 3, poi arancione→verde→blu→marrone→nera con 1/3/5/7/10 «pulite»; dan oltre la nera, +1 ogni 5). «Pulita» = criterio per-gioco deciso inline alla chiamata (di norma max 1 errore; serie ≥5 per riordina/shadowing; accordo/partita senza aiuti per appuntamento/relazioni). «Domato» = gialla o 1 pulita (banner X/15 in /giochi). Le cinture **non bloccano nulla**. Celebrazioni via `appState.beltToast`, mostrato dal layout.
5. **Sblocchi giochi** (`gameUnlocks.ts`): SOLO la filiera numeri/tempo, con requisiti espliciti mostrati sulla card (`unlockHint`, col progresso attuale): Ore+Minuti (serie 5) → Che ore sono?; +Giorni del mese (serie 5) → Data e ora (`listen-appt`, rinominato); +Dì la data (1 pulita) → Prendi appuntamento. Sblocco **morbido**: card velata 🔒 ma sempre cliccabile («provalo comunque»). Requisiti letti da gameScores (serie) e gameBelts (pulite).
