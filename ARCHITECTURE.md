...
4. **Cinture e percorso giochi** (`gameBelts.ts` → localStorage `renkei_game_belts`): contatori per gioco `{played, clean}` aggiornati da `recordGameResult(gameId, clean)` alla fine di ogni partita. La cintura deriva dai contatori (bianca 1 partita, gialla 3, poi arancione→verde→blu→marrone→nera con 1/3/5/7/10 «pulite»; dan oltre la nera, +1 ogni 5). «Pulita» = criterio per-gioco deciso inline alla chiamata (di norma max 1 errore; serie ≥5 per riordina/shadowing; accordo/partita senza aiuti per appuntamento/relazioni). `GAME_PATH` (15 stazioni ordinate) definisce il percorso: il successivo si «conquista» con gialla o 1 pulita del precedente — sblocco **morbido** (la card in /giochi è velata 🔒 ma resta cliccabile in anteprima). Celebrazioni via `appState.beltToast`, mostrato dal layout.
