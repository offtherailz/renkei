...
4. **Cinture giochi** (`gameBelts.ts` → localStorage `renkei_game_belts`): contatori per gioco `{played, clean}` aggiornati da `recordGameResult(gameId, clean)` alla fine di ogni partita (route games ai loro finali; giochi in-page di /giochi in `registerResult` al gameOver, pulita = serie ≥5). Scala karate completa: bianca 1 partita, gialla 3 partite, poi arancione/verde/blu/viola/marrone/nera con 1/2/4/6/8/10 «pulite»; dan oltre la nera +1 ogni 5 fino a 十段 «Gran Maestro». «Pulita» = criterio per-gioco deciso inline alla chiamata (di norma max 1 errore; serie ≥5 per i giochi a serie; accordo/partita senza aiuti per appuntamento/relazioni). Sblocchi e banner X/30 contano chi ha almeno l'arancione (1 pulita; `conquered`), catalogo `BELT_GAMES`. Dan mostrato come kanji dorato ricamato sulla cintura (prop `dan` di BeltIcon). Cinture disegnate da `BeltIcon.svelte` (token `--belt-*` in app.css). Le cinture **non bloccano nulla**. Celebrazioni via `appState.beltToast`, mostrato dal layout.
5. **Sblocchi giochi** (`gameUnlocks.ts`): SOLO la filiera numeri/tempo; prerequisito soddisfatto con almeno la cintura arancione (1 pulita). Catena: Ore+Minuti → Che ore sono? → Ascolta l'ora; Giorni del mese → Ascolta la data; le due → Ascolta data e ora (`listen-appt`); Ascolta data e ora + Dì la data → Prendi appuntamento; Misto quando tutta la sezione è sbloccata. `unlockHint` scrive sulla card velata cosa manca; il gioco è SEMPRE giocabile (niente vero blocco). Easter egg «7 tap veloci» in `/giochi` (`tapLocked`, finestra 700ms): sul 7° tocco lascia partire quella singola partita ma NON sblocca stabilmente la card e NON fa guadagnare cintura per quella run (`beltlessRuns` Set in-memory per i giochi in-page; per la route `/appuntamento`, flag one-shot `sessionStorage['renkei_beltless_appuntamento']` letto e consumato in `confirm()`). «Ascolta la data/l'ora» = varianti del gioco appuntamento (`generateAppointment(part)`, part 'date'|'time'|'full').

### Giochi sui numeri (`/giochi`)
Tutti a serie (un errore azzera), record per gioco via `submitScore`/`getHighscore`. Sezioni: Numeri e tempo, Ascolta e agisci, Parla, Conversazione (include «Al konbini»), Grammatica e parole, Lettura. Il 🎲 Misto (`read-mix`) pesca un round a caso da TUTTA la sezione «Numeri e tempo» (letture + Scrivi il numero + i 3 ascolti data/ora): `mixSub` (`'read'|'listen'|'appt'`) dice quale UI mostrare al giro corrente mentre `game.kind` resta `'read'`/cat `'mix'` per l'intera sessione (punteggio e cintura restano su `read-mix`); `newMixRound()` delega a `newReadRound`/`newDictation`/`newAppt` nullando gli state degli altri due per evitare che un blocco `{:else if}` stale (question/dictation/appt non azzerati) mostri la UI sbagliata.

Reattività cinture/sblocchi: `beltProgress`/`isUnlocked` leggono localStorage, non uno `$state` — senza aggancio, badge 🔒 e chip cintura in `/giochi` restavano stale finché non capitava un re-render per altro motivo (bug segnalato: sembrava che un gioco bloccato avesse dato una cintura). Fix: `appState.beltProgressVersion` (incrementato a ogni `recordGameResult`), letto da `unlockedNow`/`conqueredCountNow` e dal primo `{@const}` dello snippet `beltChip` per forzare il ricalcolo.

Voce nei giochi a contatore (日/時/分/円, Che ore sono?): `writtenVariants()` NON accetta più la forma scritta del prompt (4日/4時/3分) come prova di pronuncia — il riconoscitore normalizza qualunque lettura, giusta o "regolare"-sbagliata (lo stesso distrattore apposta generato da `counterGen.ts`: よんにち invece di よっか, よんじ invece di よじ, ふん↔ぷん, senza rendaku), nella stessa forma scritta, quindi quel bypass validava anche letture sbagliate. Tenuto solo per «Conta gli oggetti» (`generateCountObjects`), dove i distrattori sono contatori diversi (本/個/杯…) e quindi testo diverso, nessuna ambiguità. `speakReadGame`/`speakGreetGame` catturano anche `qGen` prima di `await listenJapanese()` e lo confrontano dopo (come già fa `armAfterAudio` per il TTS): senza, un round cambiato mentre il microfono ascoltava valutava la risposta sul round sbagliato. `normalizeSpeech` (speech.ts) converte anche la forma ibrida cifra-araba+contatore-kana (`digitKanaUnitToWritten`, es. «6だい»→«6台»), oltre alla forma tutta-kana (`kanaNumeralsToWritten`, es. «ろくだい»→«6台»): senza, contatori come 台 (mancante da `KANA_UNIT`) risultavano sbagliati anche a pronuncia corretta perché il riconoscitore normalizza spesso solo il numero, lasciando il contatore in kana. `kanaNumeralsToWritten` NON converte più よんじ/ななじ/きゅうじ in cifra+時 (bug segnalato: ななじ accettato per 7時): erano gli stessi distrattori "lettura regolare" apposta di counterGen.ts, e la conversione li rendeva indistinguibili da よじ/しちじ/くじ nel confronto normalizzato — la vulnerabilità viveva nella normalizzazione condivisa, non solo nel bypass già tolto da `writtenVariants`. Stesso principio esteso, dopo un test SISTEMATICO richiesto dall'utente
(`speechCounters.test.ts`, non a campione — itera OGNI valore 1-31/1-12/1-59
usando i generatori veri di counterGen.ts): **分** (ふん/ぷん non sono
intercambiabili, `MINUTE_SUFFIX_BY_ONES` indicizzata per cifra delle unità
blocca la conversione quando il rendaku non è quello lessicalizzato giusto,
es. にじゅうよんふん per 24分 restava distinta da にじゅうよんぷん) e **日**
(`DAY_NATIVE_ONLY` blocca Xにち per i 13 giorni con lettura nativa esclusiva:
1-10/14/20/24 — il test ha anche scoperto che `NATIVE_DAYS` era disallineata
da `IRREGULAR_DAYS` di counterGen.ts, mancavano 14 e 24: aggiunte). Omofono `くじ`→`工事` (「lavori」, stessa lettura) aggiunto come variante sicura in `writtenVariants` (giochi/+page.svelte): non riapre il bug perché 工事 non è una lettura numerica alternativa di nessun'altra ora. `KANA_DIGIT`/`KANA_NUM_ATOM` mancavano le forme contratte con 促音 (っ) usate nei minuti — じゅっ/ろっ/はっ/いっ (10/6/8/1 prima di ふん/ぷん, es. さんじゅうろっぷん=36分, はっぷん=8分): senza, queste letture correttamente pronunciate restavano testo kana non convertito e non convergevano mai con la cifra scritta che il riconoscitore normalmente produce.
