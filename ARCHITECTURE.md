...
5. **Sblocchi giochi** (`gameUnlocks.ts`): SOLO la filiera numeri/tempo; prerequisito soddisfatto con almeno la cintura arancione (1 pulita). Catena: Ore+Minuti → Che ore sono? → Ascolta l'ora; Giorni del mese → Ascolta la data; le due → Data e ora (`listen-appt`); Data e ora + Dì la data → Prendi appuntamento; Misto quando tutta la sezione è sbloccata. **Nessun testo spiega il requisito** in UI (card mostra solo 🔒): niente `unlockHint`, il gioco resta bloccato davvero (niente anteprima). Easter egg: `forceUnlock(id)` (localStorage `renkei_game_force_unlock`) scavalca il requisito — attivato dal trucco «7 tap veloci» in `/giochi` (`tapLocked` in +page.svelte, finestra 700ms, azzera se troppo lento). «Ascolta la data/l'ora» = varianti del gioco appuntamento (`generateAppointment(part)`, part 'date'|'time'|'full').
